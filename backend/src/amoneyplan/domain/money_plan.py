...
class MoneyPlan(Aggregate):
    ...
    @event
    def start_plan(self, initial_balance: Union[Money, float, str], 
    ...
    @event
    def allocate_funds(self, account_id: Union[UUID, str], bucket_name: str, amount: Union[Money, float, str]):
        """
        ...
        if amount > self.remaining_balance:
            raise InsufficientFundsError(f"Not enough funds to allocate {amount}. Remaining: {self.remaining_balance}")
    ...
    @event
    def reverse_allocation(self, account_id: Union[UUID, str], bucket_name: str, 
    ...
    @event
    def change_account_configuration(self, account_id: Union[UUID, str], new_bucket_config: List[BucketConfig]):
        """
    ...
    def commit_plan(self):
        ...
        if abs(total_allocated - self.initial_balance.as_decimal) >= 0.01:
            ...
            raise InvalidPlanStateError(
                f"Sum of bucket allocations ({total_allocated}) must equal initial balance ({self.initial_balance}). "
